...
- [X] T001 Paket `rpg-core/src/main/java/rpg/core/stats/package-info.java` anlegen; Javadoc hält
      die Blockgrenze fest: was B04 besitzt und was ausdrücklich B05, B06, B07, B08, B10, B11 und
      B13 gehört (FR-042)
- [~] T002 [P] ~~`StatsMessageKeys.java`~~ — **entfällt, begründet.** Message-Keys tragen
      *Spielertexte* (Prinzip V); B01, B02 und B03 nutzen sie ausschließlich für Kick-Meldungen.
      B04 hat keinen einzigen spielerseitigen Text: Konfigurationsfehler gehen als
      `ConfigValidationException` ins Betreiber-Protokoll, abgelehnte Beiträge als Ausnahme an den
      aufrufenden Block. Eine leere Schlüsselklasse anzulegen wäre Zeremonie, und ein deklarierter
      Schlüssel ohne Verwendung müsste in `messages.yml` gepflegt werden, obwohl ihn nie jemand
      liest. Nachzuholen, sobald B04 einen Spielertext bekommt.
- [~] T003 [P] ~~Textschlüssel in `messages.yml`~~ — entfällt mit T002, siehe dort.
...
- [X] T004 `rpg-core/src/main/java/rpg/core/stats/Attribute.java`: geschlossener Aufzählungstyp mit
...
- [X] T005 [P] `rpg-core/src/main/java/rpg/core/stats/AttributeKind.java` mit `ABSOLUTE` und
      `PERCENT`
- [X] T006 [P] `rpg-core/src/main/java/rpg/core/stats/UnknownAttributeException.java`; die Meldung
      nennt den unbekannten Schlüssel **und** die acht erlaubten (FR-009)
- [X] T007 `rpg-core/src/main/java/rpg/core/stats/AttributeDefinition.java` als Record mit `base`,
      `min`, `max`, `modifierBand`; der kompakte Konstruktor setzt jede Regel aus
      [data-model.md](./data-model.md) §1 durch (FR-002)
- [X] T008 `rpg-core/src/main/java/rpg/core/stats/StatConfig.java` als Record über
...
- [X] T009 [P] `rpg-core/src/test/java/rpg/core/stats/AttributeDefinitionTest.java`: jede Invariante
      aus T007 mit eigenem Fall, jeweils gegen die **Meldung** geprüft, nicht nur gegen den
      Ausnahmetyp
- [X] T010 [P] `rpg-core/src/test/java/rpg/core/stats/AttributeTest.java`: acht Konstanten, acht
...
- [X] T011 [P] [US1] `rpg-core/src/test/java/rpg/core/stats/StatCalculatorTest.java`: Basis 100 mit
      ...
      bis 3)
- [X] T012 [P] [US1] Im selben Test: Cap-Überschreitung liefert `max`, Prozentsumme unter `−100 %`
...
- [X] T013 [P] [US1] Im selben Test: `attackSpeed` mit `+200 %` bei Band `0.50` liefert
      `Basis × 1.5`; `movementSpeed` mit `−90 %` bei Band `0.30` liefert `Basis × 0.7` (FR-014)
- [X] T014 [P] [US1] `rpg-core/src/test/java/rpg/core/stats/DamageMitigationTest.java`: Verteidigung
      ...
      (FR-015, SC-006)
- [X] T015 [P] [US1] `rpg-core/src/test/java/rpg/core/stats/StatCalculatorEdgeCaseTest.java`: Wert
      `0` als Basis, als Beitrag und als Grenze; `NaN` und `Infinity` als Beitrag werden abgelehnt
      (SC-005)

### Implementierung für User Story 1

- [X] T016 [P] [US1] `rpg-core/src/main/java/rpg/core/stats/ModifierOperation.java` mit `FLAT` und
      `PERCENT` (FR-005)
- [X] T017 [P] [US1] `rpg-core/src/main/java/rpg/core/stats/SourceKind.java` mit `CLASS`, `LEVEL`,
      `EQUIPMENT`, `BUFF`, `AURA`, `ZONE`; die Reihenfolge der Konstanten **ist** die
      Summierreihenfolge (FR-006, research.md E3)
- [X] T018 [P] [US1] `rpg-core/src/main/java/rpg/core/stats/SourceId.java` als Record aus `kind`
      und `key`, gleichheitsfähig (FR-007)
- [X] T019 [P] [US1] `rpg-core/src/main/java/rpg/core/stats/StatModifier.java` als Record; lehnt
      `NaN` und `Infinity` im Konstruktor ab (FR-005)
- [X] T020 [US1] `rpg-core/src/main/java/rpg/core/stats/ModifierSet.java` als Record aus `SourceId`
      und unveränderlicher Beitragsliste; ein leerer Satz ist zulässig
- [X] T021 [US1] `rpg-core/src/main/java/rpg/core/stats/StatSnapshot.java`: hält ein privates
      `double[]`, gibt Werte ausschließlich über `get(Attribute)` heraus, trägt eine `revision` und
      bietet `isNewerThan` (FR-020, FR-022)
- [X] T022 [US1] `rpg-core/src/main/java/rpg/core/stats/StatCalculator.java`: die reine Formel
...
- [X] T023 [P] [US1] `rpg-core/src/main/java/rpg/core/stats/DamageMitigation.java`: statisch,
...
- [X] T024 [US1] `rpg-core/src/main/java/rpg/core/stats/BaseStatContributor.java` und
...
- [X] T025 [US1] Fehlerbarriere um jeden Lieferantenaufruf nach dem Muster von B01s
...
- [X] T026 [P] [US1] `rpg-core/src/test/java/rpg/core/stats/BaseStatContributorFaultTest.java`: ein
...
- [X] T027 [P] [US2] `rpg-core/src/test/java/rpg/core/stats/ModifierRoundTripTest.java`: eine Quelle
      anlegen und ablegen führt bitgleich auf den Ausgangswert zurück; 1000 Wiederholungen
      ebenfalls (FR-017, SC-004)
- [X] T028 [P] [US2] Im selben Test: dieselbe Quellenmenge in unterschiedlicher Einfügereihenfolge
      liefert bitgleiche Ergebnisse (FR-016)
- [X] T029 [P] [US2] `rpg-core/src/test/java/rpg/core/stats/ModifierSourceTest.java`: Entfernen
      ...
      Szenarien 3, 5, 6)
- [X] T030 [P] [US2] `rpg-core/src/test/java/rpg/core/stats/ContributionQueryTest.java`:
...
- [X] T031 [US2] `rpg-core/src/main/java/rpg/core/stats/StatHolder.java`: Träger mit
      `LinkedHashMap<SourceId, ModifierSet>`, `volatile StatSnapshot`, Revisionszähler und
      `characterId` als nullbarem Feld (FR-035)
- [X] T032 [US2] `rpg-core/src/main/java/rpg/core/stats/StatEngine.java`: die öffentliche
...
- [X] T033 [US2] `rpg-core/src/main/java/rpg/core/stats/DefaultStatEngine.java`: `apply`,
...
- [X] T034 [US2] Summierung in fester Reihenfolge — sortiert nach `SourceKind`, dann nach
      Quellenschlüssel — damit die Einfügereihenfolge das Ergebnis nicht beeinflusst (FR-016,
      research.md E3)
- [X] T035 [US2] `contributions(UUID, Attribute)` und `AttributeContribution` ergänzen; liest nur
      die Quellenkarte (FR-010)
- [X] T036 [US2] `createForCharacter`, `createForEntity`, `remove` und `recalculateNow` in
...
- [X] T037 [P] [US3] `rpg-core/src/test/java/rpg/core/stats/RecalculationBudgetTest.java`: sechs
...
- [X] T038 [P] [US3] Im selben Test: 200 Träger mit je 20 Quellen über 1200 Ticks ohne Änderung
      ergeben **null** weitere Neuberechnungen und **null** geplante Aufgaben (FR-018, SC-002)
- [X] T039 [P] [US3] Im selben Test: 100 Träger ändern im selben Tick, Gesamtdauer unter 5 ms
      (SC-003)
- [X] T040 [P] [US3] `rpg-core/src/test/java/rpg/core/stats/SnapshotImmutabilityTest.java`: ein
...
- [X] T041 [P] [US3] `rpg-core/src/test/java/rpg/core/stats/RecalcPendingTest.java`: eine Abfrage
...
- [X] T042 [US3] Vormerkung `recalcPending` als `AtomicBoolean` in `StatHolder`; die erste Änderung
...
- [X] T043 [US3] `recalculateNow` überspringt die Bündelung, rechnet sofort und löscht die
      Vormerkung; genutzt vom Ladepfad und von Trägern ohne Entität (FR-019b)
- [X] T044 [US3] Die geplante Aufgabe prüft vor dem Rechnen, ob der Träger noch existiert, und
      kehrt sonst folgenlos zurück
- [X] T045 [P] [US3] `rpg-core/src/main/java/rpg/core/stats/StatsRecalculatedEvent.java` als Record
      mit vorigem und neuem Schnappschuss (FR-023)
- [X] T046 [US3] Veröffentlichung des Ereignisses über `EventBus` **nach** dem Anstoß der
      Vanilla-Spiegelung, damit kein Abonnent Wert und Anzeige auseinanderlaufen sieht
      ([contracts/events.md](./contracts/events.md))
- [X] T047 [US3] `rpg-persistence/src/main/java/rpg/persistence/stats/StatsModule.java`: Modul nach
...
- [X] T048 [US3] `StatsModule` in `rpg-plugin/src/main/java/rpg/plugin/RpgPlugin.java` registrieren
...
- [X] T049 [P] [US4] `rpg-platform/src/test/java/rpg/platform/stats/VanillaAttributeBridgeTest.java`:
...
- [X] T050 [P] [US4] Im selben Test: 0,4 von 1000 Leben ergeben `0.5` angezeigte Punkte, 0 von 1000
      ergeben `0.0` (FR-031, SC-008, Szenarien 3 und 4)
- [X] T051 [P] [US4] Im selben Test: eine Änderung an `movementSpeed` oder `attackSpeed` setzt das
...
- [X] T052 [P] [US4] `rpg-platform/src/test/java/rpg/platform/stats/VanillaRegenerationGuardTest.java`:
      `naturalRegeneration` steht auf `false`, die Sättigung wird festgehalten (FR-030a)
- [X] T053 [P] [US4] `rpg-platform/src/test/java/rpg/platform/stats/NoDamageInterceptionTest.java`
...
- [X] T053a [P] [US4] `rpg-platform/src/test/java/rpg/platform/stats/MirrorBeforeEventTest.java`:
...
- [X] T054 [P] [US4] `rpg-core/src/main/java/rpg/core/stats/VanillaAttributeBridge.java` als
      Schnittstelle mit `mirrorHealth`, `mirrorAttackSpeed`, `mirrorMovementSpeed`; ohne
      Registrierung wirkungslos (FR-034)
- [X] T055 [US4] `rpg-platform/src/main/java/rpg/platform/stats/PaperVanillaAttributeBridge.java`:
      ...
      (FR-030 bis FR-033)
- [X] T056 [US4] `rpg-platform/src/main/java/rpg/platform/stats/VanillaRegenerationGuard.java`:
...
- [X] T057 [US4] Brücke und Wächter in `RpgPlugin` aufbauen, dem `StatsModule` übergeben und den
...
- [X] T058 [P] [US5] `rpg-core/src/test/java/rpg/core/stats/ResourcePoolTest.java`: steigendes
...
- [X] T059 [P] [US5] Im selben Test: eine Änderung um null veröffentlicht **kein** Ereignis; das
      Klemmen durch ein gesunkenes Maximum trägt die Ursache `CLAMPED_BY_MAX`
      ([contracts/events.md](./contracts/events.md))
- [X] T060 [P] [US5] `rpg-persistence/src/test/java/rpg/persistence/stats/CharacterStatsMigrationTest.java`:
...
- [X] T061 [P] [US5] `rpg-persistence/src/test/java/rpg/persistence/stats/CharacterResourcesRoundTripTest.java`:
      ...
      vollem Stand (SC-011)
- [X] T062 [P] [US5] `rpg-persistence/src/test/java/rpg/persistence/stats/NoWritePerEventTest.java`:
      500 Ressourcenänderungen in einer Sitzung erzeugen genau einen Schreibvorgang je
      Flush-Zyklus, nicht 500 (FR-028, SC-012)
- [X] T063 [P] [US5] `rpg-core/src/test/java/rpg/core/stats/SessionNotReadyTest.java`: Abfragen und
...
- [X] T063a [P] [US5] `rpg-core/src/test/java/rpg/core/stats/EntityHolderBypassesSessionTest.java`:
...
- [X] T064 [P] [US5] `rpg-core/src/main/java/rpg/core/stats/ResourcePool.java` mit den Klemmregeln
      aus [data-model.md](./data-model.md) §5 (FR-025, FR-026)
- [X] T065 [P] [US5] `rpg-core/src/main/java/rpg/core/stats/ResourceChangedEvent.java` samt
      `ResourceKind` und `ChangeCause` (FR-029)
- [X] T066 [US5] `changeHealth`, `changeMana` und `resources` in `DefaultStatEngine`; Rückgabe ist
      der Stand **nach** dem Klemmen, damit der Aufrufer den tatsächlichen Verbrauch erfährt
- [X] T067 [US5] Das Klemmen bei sinkendem Maximum an die Neuberechnung koppeln, mit Ursache
      `CLAMPED_BY_MAX` — und ausdrücklich **ohne** Todesereignis (Blockgrenze zu B05)
- [X] T067a [US5] `DefaultStatEngine` erhält die `SessionRegistry` aus B03 als Abhängigkeit und
...
- [X] T068 [P] [US5] `rpg-core/src/main/java/rpg/core/stats/CharacterResources.java` als
      persistierbarer Record und
      `rpg-core/src/main/java/rpg/core/stats/CharacterResourcesRepository.java` als Schnittstelle
- [X] T069 [US5] `AggregateType.CHARACTER_STATS` in
      `rpg-core/src/main/java/rpg/core/persistence/AggregateType.java` ergänzen
- [X] T070 [US5] `FlushCycle.WRITE_ORDER` in `rpg-persistence/src/main/java/rpg/persistence/FlushCycle.java`
...
- [X] T071 [US5] `rpg-persistence/src/main/resources/db/migration/V4_1__character_stats.sql` nach
...
- [X] T072 [US5] `rpg-persistence/src/main/java/rpg/persistence/stats/JdbcCharacterResourcesRepository.java`
      nach dem Muster von `JdbcCharacterRepository`, mit Stapelschreiben und Revisionsprüfung
- [X] T073 [US5] `rpg-core/src/main/java/rpg/core/session/SessionBundle.java` um
      `List<CharacterResources>` erweitern; `empty(...)` und die Prüfungen im kompakten Konstruktor
      mitziehen
- [X] T074 [US5] `rpg-persistence/src/main/java/rpg/persistence/session/SessionBundleLoader.java` um
...
- [X] T075 [US5] Ladepfad in `StatsModule`: aus dem Bündel einen Träger je Charakter aufbauen,
      `recalculateNow` aufrufen und erst danach freigeben — kein Spieler wird mit ausstehender
      Vormerkung bereit gemeldet (FR-019b)
- [X] T076 [US5] Entladepfad: beim Sitzungsende Träger entfernen und den Ressourcensatz über
...
- [X] T077 [P] [US6] `rpg-core/src/test/java/rpg/core/stats/EntityHolderTest.java`: ein Träger ohne
      Spielerbezug liefert bei gleicher Quellenmenge bitgleich dieselben Werte wie ein
      Spielerträger (FR-035, Szenario 1)
- [X] T078 [P] [US6] Im selben Test: nach `remove` sind Quellen, Schnappschuss und Vormerkung
      freigegeben und die interne Trägerkarte enthält keinen Eintrag mehr (FR-036, Szenario 2,
      SC-010)
- [X] T079 [P] [US6] Im selben Test: 800 Träger ohne Spielerbezug im Leerlauf erzeugen keine
      geplante Aufgabe und keine Neuberechnung (Szenario 3)
- [X] T080 [P] [US6] `rpg-core/src/test/java/rpg/core/stats/HolderLifecycleLeakTest.java`: nach 200
      angelegten und wieder entfernten Sitzungsträgern ist die Trägerkarte leer (SC-010)

### Implementierung für User Story 6

- [X] T081 [US6] `createForEntity` ohne `characterId` in `DefaultStatEngine` vollständig
      ausformulieren; ein Träger ohne `characterId` wird nie persistiert und nie vorgemerkt
      (FR-035)
- [X] T082 [US6] `remove` gibt Quellenkarte, Schnappschussverweis und Vormerkung frei und ist
...
- [X] T083 [P] [US7] `rpg-core/src/test/java/rpg/core/stats/StatConfigValidationTest.java`: je ein
      ...
      FR-014a, SC-009)
- [X] T084 [P] [US7] Im selben Test: ein fehlendes Attribut wird namentlich benannt; ein unbekannter
      Schlüssel nennt die acht erlaubten (User Story 7 Szenario 3, FR-004a)
- [X] T085 [P] [US7] `rpg-core/src/test/java/rpg/core/stats/StatConfigReloadTest.java`: ein
...
- [X] T086 [US7] `ConfigSchema<StatConfig>` in `StatConfig` ergänzen, nach dem Muster von
      `SessionConfig`; jede der neun Regeln aus dem Vertrag wird durchgesetzt
- [X] T087 [US7] `rpg-plugin/src/main/resources/stats.yml` mit den Auslieferungswerten aus
      [contracts/stat-config.md](./contracts/stat-config.md), mit Kommentaren auf Englisch
- [X] T088 [US7] `stats.yml` in `RpgPlugin.DEFAULT_CONFIG_FILES` aufnehmen, damit die Datei beim
      ersten Start angelegt wird und Betreiber-Änderungen jeden Neustart überleben
- [X] T089 [US7] `StatsModule` lädt die Konfiguration über `ConfigLoader.register` statt
      `StatConfig.defaults()` zu verwenden; ein Fehler bricht den Start ab
- [X] T090 [US7] Beim Nachladen alle bekannten Träger vormerken — der einzige Fall, in dem B04 über
...
- [X] T091 `rpg-plugin/src/test/java/rpg/plugin/FullBootstrapTest.java` erweitern: `StatsModule` ist
...
- [X] T092 Abschnitt 0 des Validierungsleitfadens ausführen: alle XML-Testberichte auf
...
- [X] T093 `./gradlew spotlessApply build` und den vollständigen Testlauf grün stellen
- [X] T094 [P] ADR-013 in `02-decisions.md` ergänzen: die Umsetzungsentscheidungen aus
...
- [X] T095 [P] Status in `minecraft-rpg-spec/minecraft-rpg-spec/blocks/B04-stat-engine.md` auf
      implementiert setzen und auf `specs/004-stat-engine/` verweisen
- [X] T096 [P] `06-open-questions.md`: den B04-Abschnitt als abgeschlossen markieren und die fünf
      Klärungen aus dieser Runde eintragen
- [X] T097 Abschnitte 1 bis 7 des Validierungsleitfadens vollständig durchlaufen
