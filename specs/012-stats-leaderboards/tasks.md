...
- [X] T144 Deploy auf den Testserver *(2026-08-30)* — **`statistics.yml` UND `messages.yml` mussten von Hand mit.** Die Aufgabe nannte nur die erste; die zweite ist vorhanden, aber um 46 Schluessel gewachsen, und der Start bricht gegen sie ab, BEVOR eine B12-Pruefung ueberhaupt laeuft. Genau so passiert. Ein Vergleich aller YAMLs (mit `--strip-trailing-cr`, sonst meldet CRLF-vs-LF jede Datei) zeigt: nur diese beiden weichen wirklich ab. `persistence.yml` unberuehrt
- [X] T145 Startpruefungen auf dem Testserver **absichtlich provoziert** *(2026-08-30)*, alle drei brechen mit Datei, Schluessel und Grund ab: Saisonluecke (`statistics.yml: zwischen den Saisons '2026-q3' und '2026-q4' klafft eine Luecke - 2026-10-01 bis 2026-10-04 gehoert zu keiner Saison`), leere Gewichtung (`score.weights fehlt oder ist leer`), Schwelle 0 (`capture.kill-credit-share ist 0.0 - erlaubt ist echt zwischen 0 und 1`)
- [X] T146 Echter Serverstart als Beleg fuer den Klassenlader *(2026-08-30)* — Paper 26.2, Java 25: Plugin geladen, Flyway auf 12.3 (17 Migrationen), `ready for players after 1142ms`, Server oben in 7,8s, sauberes Herunterfahren, **null Fehler oder Warnungen von uns**. Zusaetzlich belegt: das Hologramm wird auf echtem Paper als `TextDisplay` gesetzt (keine `could not set`-Warnung, also traegt Paper alle Haertungsschritte), und der **Auffrischungstakt laeuft wirklich** — eine Rohzeile, die vor dem Start in keiner der drei Materialized Views stand, war nach zwei Takten in allen dreien. Das belegt `REFRESH ... CONCURRENTLY` samt dem Commit gegen den echten Pool mit `autoCommit=false`
- [ ] T147 Nach einer Spielstunde in die Tabelle sehen: Schlüssel tragen ihre Dimension, und die Summe der Zonenzeiten entspricht der aktiven Gesamtzeit ([quickstart.md](./quickstart.md) §5) — **halb belegt am 2026-08-30**: die Zeitrechnung stimmt (`playtime_active.greenfields/darkforest/pale-wilds/wilderness` tragen ihre Dimension, Summe 758 s gegen `playtime_online` 1282 s, die Differenz ist exakt der AFK-Block), die Kills und Tode sind es **nicht** — siehe T149 bis T151
- [ ] T148 `quickstart.md` von oben nach unten durchlaufen und abhaken

---

## Phase 10: Was der erste echte Spielerlauf gefunden hat *(2026-08-30)*

Drei Fehler, die 2837 grüne Tests nicht gesehen haben. Gefunden beim Versuch, T147 abzuhaken —
und **alle drei erst auf dem Server sichtbar**, weil sie an der Grenze zwischen unseren Doubles und
der Wirklichkeit sitzen. Keiner davon hält T147 formal auf; sie machen T147 unbelegbar, was
schlimmer ist.

- [ ] T149 **Kreaturen schreiben in die Spielerstatistik.** `DamageStatListener.onDamage` (`rpg-platform/src/main/java/rpg/platform/statistics/DamageStatListener.java:54`) prüft nur, ob es einen Angreifer *gibt*, und schreibt dessen UUID als Konto — ob ein Spielerkonto dahintersteht, fragt niemand. Befund auf dem Testserver: **56 `damage_max`-Zeilen mit 56 verschiedenen `player_id`**, genau eine davon ein Spieler; die anderen 55 sind Kreaturen mit Werten wie 2, 3, 4. `data-model.md §1.1` sagt „Konto, nicht Charakter (Fund 3, FR-005)" — hier stehen Entitäten. Und da **nichts** aus dieser Tabelle je gelöscht wird (ADR-044), wächst der Müll mit jedem Schlag, den eine Kreatur austeilt. Fix: gegen `AccountLookup` prüfen, bevor geschrieben wird. Test: ein Mob-Angreifer erzeugt **keine** Zeile — mit einer echten Angreifer-UUID, die kein Konto ist
- [ ] T150 **Tode durch unbekannte Verursacher landen unter `deaths.player`.** `KillStatListener.isPlayerLike` (`rpg-platform/src/main/java/rpg/platform/statistics/KillStatListener.java:164`) ist `event.shares().shares().containsKey(event.killerId())` — und wer den tödlichen Schlag führt, steht **immer** in den Shares, Spieler wie Kreatur. Die Prüfung ist konstant wahr, der Rückfall auf `deaths.environment` greift nie. Der Kommentar darüber beschreibt die Absicht korrekt; der Code erfüllt sie nicht. **Der Test ist grün, weil er die Welt falsch baut**: `UnresolvableKillerFallsBackTest:58` übergibt `DamageShare.empty()` — eine Lage, die es im Spiel nicht gibt, wenn ein Verursacher bekannt ist. Fix und Test gehören zusammen: erst den Test auf echte Shares umstellen (er wird rot), dann `isPlayerLike` reparieren
- [ ] T151 **Kills werden nach einem Serverneustart nie gebucht — zwei Wahrheiten über dieselbe Kreatur.** Belegt am 2026-08-30: ein Kill gab Erfahrung, erzeugte aber **keine** `mob_kills`-Zeile. B06 fragt das Tag an der Entität (`ProgressionDeathListener:66` → `MobKindTag.kindKeyOf` → `PersistentDataContainer`), das mit der Entität in der Weltdatei liegt und den Neustart überlebt. B12 fragt die Registry (`KillStatListener:95` → `MobKinds.ofEntity` → `HordeRegistry`), eine reine `LinkedHashMap` **ohne Wiederherstellung**. Nach jedem Neustart ist jede überlebende Kreatur für B12 namenlos: `kind.isEmpty()`, sofortiger Ausstieg, kein Kill. Dieselbe Wurzel trägt T150 (auch `causeKey` fragt die Registry) und vermutlich die `[item] could not drop loot for a death`-Warnungen aus B11. **Entschieden am 2026-08-30, [ADR-050](../../02-decisions.md): die Ursache wird in B10 behoben, nicht in B12.** Eine getaggte Kreatur, die beim Chunk-Laden nicht in der Registry steht, wird **entfernt**; sie respawnt aus dem Budget. Nicht adoptiert — ein `Entry` bräuchte die Ursprungszone (FR-017) und `spawnedAt`, beides nach einem Neustart nicht rekonstruierbar. **Der Bau gehört auf einen B10-Branch, nicht hierher.** In B12 bleibt nur T150 zu tun; danach ist der hier beschriebene Ausfall keiner mehr. Vor der Umsetzung eine Zahl erheben: die Registry zur Laufzeit gegen die lebende Population halten — der Verdacht, dass das Budget umgangen wird, ist bislang ungemessen
- [ ] T152 Nach T149 bis T151: T147 **von vorn** durchlaufen — die bisherigen Zeilen sind kein Beleg. Vorher `rpg.player_statistic_daily` leeren, sonst stehen die 55 Kreaturenzeilen für immer darin
