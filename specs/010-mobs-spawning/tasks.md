...
- [ ] T007 [P] `HordeSpec` und `HordeEntry` in `rpg-core/src/main/java/rpg/core/mob/HordeSpec.java` — `entries` nicht leer, `weight` ≥ 1
...
- [ ] T009 [P] `Budget` in `rpg-core/src/main/java/rpg/core/mob/Budget.java` — vier Grenzen, dazu `boolean allows(...)`, das **die schärfste** entscheiden lässt; Javadoc sagt, dass es eine Grenze ist und kein Zielwert (FR-013)
...
- [ ] T015 `MobConfig` in `rpg-core/src/main/java/rpg/core/mob/MobConfig.java` — der validierte Inhalt von `mobs.yml`: Budgets, Horden-Werte, Arten, Horden, `suppressVanilla`
...
- [ ] T033 [US1] Dieselbe Umstellung an der Erfahrungsseite — jede Stelle finden, die heute `getType().name()` benutzt, und über `kindKeyOf` führen
...
- [ ] T045 [US2] Der Durchlauf läuft nicht in einer Zone ohne Spieler (FR-015) — geprüft **bevor** irgendetwas gerechnet wird
- [ ] T046 [US2] `PaperMobPlacer` an den Durchlauf hängen — Setzen ortsgebunden über B01s Scheduler, nie über den globalen (Prinzip I)
- [ ] T047 [US2] Bestand nachführen beim Setzen: Zone, Chunk, gesamt (FR-013, FR-017)
...
- [ ] T057 [US2B] `suppress-vanilla` in `mobs.yml` verdrahten — der Schalter gilt für das ganze Plugin, nicht je Welt (contracts/mob-config.md)
- [ ] T058 [US2B] Log-Zeile beim Start: welche Regeln auf wie vielen Welten gesetzt wurden — damit auf dem echten Server nachvollziehbar ist, ob Schicht 1 überhaupt griff
...
- [ ] T062 [US3] `CleanupRule` in `rpg-core/src/main/java/rpg/core/mob/CleanupRule.java` — die Entscheidung, ohne Bukkit; Frist und Reichweite aus der Konfiguration
- [ ] T063 [US3] Kampfzustand über B05 abfragen statt selbst zu führen (FR-022) — B05 rechnet ihn lazy aus Zeitstempeln, eine zweite Buchführung wäre eine zweite Wahrheit
- [ ] T064 [US3] Aufräumen in `HordeSweep` einhängen — **derselbe Durchlauf** wie das Nachsetzen, nicht ein zweiter (research.md R4)
- [ ] T065 [US3] Entfernen entitätsgebunden über B01s Scheduler, und den Bestand dabei nachführen
...
- [ ] T072 [US4] Nachschubrate ebenfalls skalieren, aus `horde.respawn-interval-ms` und `density-per-player`
- [ ] T073 [US4] Spielerzahl je Zone über B09s Anwesenheit ermitteln, nicht über eine eigene Zählung
- [ ] T074 [US4] Sinkende Spielerzahl: Zieldichte fällt, der Überhang wird über das Aufräumen abgebaut und **nicht sofort gelöscht** (FR-028)
...
- [ ] T080 [US5] Bossplatzierung über den `SpawnArea`-Schlüssel plus Versatz (research.md R10) — **keine eigenen Weltkoordinaten**
- [ ] T081 [US5] Boss in den Durchlauf einhängen: erscheint, wenn keiner lebt, Timer abgelaufen ist und Spieler da sind
- [ ] T082 [US5] Bosstod erkennen und `lastKilledAt` setzen — über dasselbe Todesereignis wie alles andere, keine Sonderbehandlung
- [ ] T083 [US5] Aufräumen behandelt den Boss wie jede Kreatur, ohne den Timer anzufassen (FR-034)
...
- [ ] T088 [US6] Test `HordeBudgetBenchmarkTest` in `rpg-core/src/test/java/rpg/core/mob/HordeBudgetBenchmarkTest.java` — **FR-038, SC-007**: ein Spawn- und ein Aufräum-Durchlauf bei 130 Kreaturen, zusammen unter 1 ms; Kopfkommentar sagt ausdrücklich, dass dies **kein Lasttest** ist und der Nachweis unter Volllast zu B15 gehört (ADR-031). Vorbild: `ZoneLookupBenchmarkTest`

### Umsetzung

- [ ] T089 [US6] `follow-range` je Art über `Attribute.FOLLOW_RANGE` setzen, in `PaperMobPlacer` (FR-036, research.md R6)
- [ ] T090 [US6] Drosselung der eigenen Zielzuweisung als Regel in `rpg-core`, zeitstempelbasiert lazy (FR-035, Prinzip II)
...
- [ ] T096 [US7] Erst fragen, ob überhaupt ein Klon steht — eine Karte, die fast immer leer ist, und deshalb kostet der Zuhörer für alle anderen nichts
...
- [ ] T101 `MobMessageKeys.all()` in die Startprüfung der Message-Schlüssel aufnehmen
