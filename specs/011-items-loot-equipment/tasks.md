...
- [ ] T087 [US4] Verkauf verdrahten — Erlös unter `BookingReason.VENDOR_SALE` (CREDIT), Kauf unter `VENDOR_PURCHASE` (DEBIT). **Die Spec hatte beide vertauscht:** aus Sicht des Spielers ist Verkaufen eine Gutschrift, und `VENDOR_PURCHASE` ist der Kauf. Beim Nachsehen in `BookingReason` aufgefallen (FR-060)
